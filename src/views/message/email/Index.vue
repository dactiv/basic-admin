<template>
  ...
  <a-modal v-model:visible="searchDialogVisible" width="600px" title="查询邮件管理" @ok="search(null)" layout="vertical">
      ...
      <a-row :gutter="[24]">
        <a-col :span="12">
          <a-form-item label="类型:">
            <a-input v-model:value="form['filter_[type_eq]']" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
...
export default {
  ...
  methods:{
    send() {
      this.$router.push({name:"email_send"});
    },
    detail(record) {
